...
////END STEMMER CODE, BEGIN ORIGINAL CODE////


	$term   = $_POST["query"];
	$term_s	= PorterStemmer::stem($term);
	$type	= "type=topic";
	$limit 	= "limit=10";
	$args	= "$type&$limit";
	$url	= "https://hackproxy.whisper.sh/search/suggest/$term?$args";
	$ch		= curl_init();
	$lat	= 34;
	$lon	= 118;
	$limit	= 10;
	...
	foreach ($data as $value) {
		$stem = PorterStemmer::stem($value->name);
		if($stem == $term_s)
			$feeds[] = array($value->name, $value->id);
	}
	//var_dump($feeds);
	//END code for getting feed id based upon place
$locations = array();
foreach ($feeds as $value) {
	...
	for($i = 0; $i < 1; $i++)
	{
		...
		$limit 	= "limit=50";
		$feed_id = "feed_id=$value[1]";
		$args	= "$limit&$feed_id&before_wid=$before_wid";	
		$url	= "https://hackproxy.whisper.sh/feeds/whispers/$term?$args";
		$ch		= curl_init();
